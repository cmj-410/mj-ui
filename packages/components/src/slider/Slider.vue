<template>
  <div
    class="mj-slider"
    :class="{ 'mj-slider--disabled': disabled }"
    :tabindex="disabled ? -1 : 0"
    @keydown.down.left.stop.prevent="keyEvent(Positions.L)"
    @keydown.up.right.stop.prevent="keyEvent(Positions.R)"
  >
    <span v-if="labelTop" class="mj-slider-label">{{ labelTop }}</span>
    <div
      ref="sliderLineRef"
      class="mj-slider-line"
      @mouseenter="handleMouseEnter"
      @mouseleave="handleMouseLeave"
      @click="onSliderClick"
      @touchend="onSliderClick"
    >
      <div class="mj-slider-bar" :style="{ width: `${valuePosition}%` }"></div>
      <div
        class="mj-slider-controller-wrapper"
        :style="{ left: `${valuePosition}%` }"
        @mousedown="onMouseOrTouchDown"
        @touchstart.prevent="onMouseOrTouchDown"
      >
        <mj-tooltip ref="tooltipRef" :content="modelValue">
          <div class="mj-slider-controller"></div>
        </mj-tooltip>
      </div>
    </div>
    <div v-if="stepPoints && stepsPoints.length" class="mj-slider-wrap-points">
      <div
        v-for="(step, index) in stepsPoints"
        :key="step.left"
        class="mj-slider-point"
        :style="getStepPointStyles({ step, index })"
        :class="{ 'mj-slider-point--active': step.active }"
      ></div>
    </div>
    <div v-if="numbers" class="mj-slider-numbers">
      <div>{{ min }}</div>
