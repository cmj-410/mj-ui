    ...
    <transition name="fade">
      ...
      >
        <transition name="drop-top">
          <div v-show="modelValue" class="mj-modal-wrap">
            <div
              class="mj-modal-wrap-inner"
              tabindex="0"
            ...
            >
              ...
              >
                <slot name="image" />
                <slot />
                <div v-if="itHasHeader" class="mj-modal-header">
                  <slot name="header" />
                </div>
                <div v-if="itHasBody" class="mj-modal-content">
                  <slot name="body" />
                </div>
                <div v-if="itHasActions" class="mj-modal-footer">
                  <slot name="actions" />
                </div>
              </div>
            </div>
  ...
  setup(props, { emit, slots }) {
    ...
    watch(
      ...
      async (active: boolean) => {
        await nextTick()
        const modalsList = MJUI!.appContext.config.globalProperties.$MJUI.modals
        if (modalRef.value) {
          if (active) {
